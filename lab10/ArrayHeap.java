...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (parentIndex(index) != 0 && getNode(parentIndex(index)).priority() > getNode(index).priority()) {
    ...
    private void sink(int index) {
        ...
        if (left != null && right != null && current.priority() > left.priority() && current.priority() > right.priority()) {
            int min = min(leftIndex(index), rightIndex(index));
            swap(index, min);
            sink(min);
            return;
        }
        if (left != null && current.priority() > left.priority()) {
            swap(index, leftIndex(index));
            sink(leftIndex(index));
            return;
        }
        if (right != null && current.priority() > right.priority()) {
